...
| `is_sched:settings` | 业务数据·单例 | 全局参数对象（`dailyTaskLimit`/`slotTaskLimit`/`warnDailyCount` 等，读取时与 `DEFAULT_SETTINGS` 合并；**单例对象非数组表**，不经 db Map，store 层直读写） | store.getSettings / saveSettings | 清除 | 导出/导入 |
...
3. **重置语义**：`resetAll` 清三表业务数据 + 清除系统参数（settings）——「重置 = 回全新默认环境」；UI 状态（视图记忆/侧栏）跨重置保留，属本机偏好。
4. **备份语义**：JSON 备份/恢复含业务三表 + 系统参数（settings，单例对象手动并入导出负载）；UI 状态不随备份迁移（本机偏好）。恢复时旧备份文件若无 `settings` 字段，保留导入方本机当前参数（向后兼容）。
