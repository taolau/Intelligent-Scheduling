...
## 46. 中文文本在 flex 里没有 nowrap 会被压成「一个字宽」的竖排；修布局先量「需要 vs 可用」再动手

- **报错**：配置页任务卡片脚注的三个按钮（加到全员 / 删除 / 编辑）文字**逐字竖排**，开关标签「启用」也竖成两行
- **场景**：卡片最小列宽 288px（内容宽 254px），而脚注一行要放「开关 + 三个带图标按钮」。窗口稍窄、卡片贴到最小列宽即触发；其他宽度下卡片更宽，完全看不出来（所以极易被当成"偶发"）
- **根因**：两层——① **`.btn` 与 `.switch-wrap` 都没写 `white-space:nowrap`**：中文任意两字之间都允许断行，flex 子项 `min-width:auto` 解析出的 min-content 宽度 = **一个字宽**，空间不足时 flex 把按钮逐字压扁成竖排（拉丁文单词不可断行，不会这样）；② 真正的根因是**这一行本来装不下**——只给按钮加 nowrap 只是把「竖排」换成「折行」，开关标签照样继续竖排（第一轮就栽在这：修了症状，没量账）
- **解决**：先量账——按钮组 239 + 间距 8 + 开关 67 = **314px**，对 288px 最小列宽（可用 254px）缺 60px；故 ① `.card-grid` 最小列宽 **288→360**（给出 326px，余量 12px）② `.switch-wrap` 加 `flex-shrink:0; white-space:nowrap` ③ `.btn` 加 `white-space:nowrap`（兜底）④ `.cfg-op-btns` 加 `flex-wrap:wrap; justify-content:flex-end`（真挤不下时整组折行、绝不压扁）。验收用 Playwright 把网格列宽**钉死在 360px** 再量（溢出/按钮 top 是否同行/标签高是否单行）
- **启示**：① **中文 + flex + 无 nowrap = 竖排**——凡「中文标签/按钮放进 flex 行」一律显式 nowrap（或 `flex-shrink:0`），与 #26/#38 同族（都是 flex 内元素对齐/尺寸被默认值坑）；② 「挤压型」布局 bug 要**先算宽度账**（需要多少 px、可用多少 px、差多少），只堵一个症状必漏同行的其他元素；③ 最小列宽是**算出来的**不是拍出来的——日后往这一行加按钮或加长文案，必须同步复核那个最小值（CSS 注释里已留账）；④ UI 回归要测**极限尺寸**：把容器尺寸钉死在临界值再量，比拖窗口碰运气可靠
