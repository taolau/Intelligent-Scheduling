...
| | `substitute.js` | buildContext 上下文聚合（周/月/累计三轨计数）+ cloneCtx 导出 + 替补 Top3 推荐 | filter/score/week |
| | `auto.js` | 智能排班自动填充纯函数（simulateAutoFill 按序模拟决策不写库 + accumulateDelta 六轨计数增减规则唯一实现） | filter/score/substitute/week |
| `views/` | `calendar.js` | 排班视图（核心交互；**周/月双粒度切换** 状态 is_sched:cal_scale，月 = 覆盖自然月的完整周面板纵向堆叠、非本月日灰显；**视图维度切换**：总览/项目/人员过滤+摘要，状态 is_sched:cal_view，**人员维度标签多选过滤**（页内临时态）工具栏下拉；工具栏**「导出图片」按钮**（周=当前周面板、月=整月长图，文件名带粒度+维度）；**智能排班 smartPlanDialog**（预览=core simulateAutoFill 模拟执行+确认批量落盘，范围=当前浏览范围）+ 闪电单班次；**批量铺排 bulkPlanDialog**（previewExpand 预览+确认）；**排班分配/替换弹窗候选标签过滤**（视图层，算法名单不变）；**替换弹窗 openReplaceDialog**（当天班次分组按时段排序、候选卡、完成态折叠绿条） | core+data+ui |
...
| | `fields.js` | 表单构建：field()/setError()/rowsEditor() 结构化动态行 + **tagsInput() 标签 chip 输入**（同框可输可选：聚焦浮出已有标签候选、回车自创、去重） | select |
